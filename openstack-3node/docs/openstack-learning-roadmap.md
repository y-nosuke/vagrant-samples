...
- **インターフェース**: eth0
- **選定理由**:
  - ホストOSからVMへの直接アクセスが必要（SSH、API、Horizonダッシュボード）
  - 開発・デバッグ・管理の利便性を優先
  - 管理トラフィックはセキュリティ要件が比較的緩い（プライベートネットワーク内）
...
- **インターフェース**: eth1
- **選定理由**:
  - テナント通信（VM間通信）を完全に隔離する必要がある
  - ホストOSからテナント通信を直接見えないようにする（セキュリティ向上）
  - オーバーレイネットワークは通常ホストから直接アクセスしない設計
  - トラフィック分離により管理ネットワークと競合しない
...
- **選定理由**:
  - Floating IPの動作確認には外部からの直接アクセスが必要
  - 物理ネットワーク上のデバイスとVMが同じセグメントで通信できる（本番環境に近い）
  - プロバイダーネットワーク（FLAT）の動作確認に適している
  - NAT Networkだと外部からVMへの直接アクセスが難しいため不適切
...
> **📌 注意**: ネットワーク構成の詳細は [🌐 ネットワーク構成](#-ネットワーク構成) セクションを参照してください。
