...
- [Part 2: OpenStackアーキテクチャの理解](#part-2-openstackアーキテクチャの理解)
  ...
  - [1. OpenStackの主要コンポーネント](#1-openstackの主要コンポーネント)
    - [1.1 Keystone（認証・認可サービス）✅ 🎓🏢](#11-keystone認証認可サービス-)
      - [主な機能](#主な機能)
      - [主要概念 ⭐](#主要概念-)
      - [OpenStackの基本概念（プロジェクト、ユーザー、ロール、ドメイン）⭐](#openstackの基本概念プロジェクトユーザーロールドメイン)
        - [プロジェクト（Project）とは何か](#プロジェクトprojectとは何か)
        - [マルチテナントとは何か](#マルチテナントとは何か)
      - [認証フロー 💡](#認証フロー-)
...
#### OpenStackの基本概念（プロジェクト、ユーザー、ロール、ドメイン）⭐

> **📌 重要**: プロジェクト、ユーザー、ロール、ドメインは**OpenStack全体の概念**です。Keystoneが管理しますが、Nova、Glance、Neutron、Cinderなど、すべてのOpenStackサービスで使用されます。

##### プロジェクト（Project）とは何か

> **💡 基礎知識**: プロジェクト（Project）は、OpenStackにおける**リソースのグルーピング単位**です。旧称は「テナント（Tenant）」と呼ばれていました。

**プロジェクトの役割**:

- **リソースの分離**: 仮想マシン、ストレージ、ネットワークなどのリソースをグループ化
- **アクセス制御**: プロジェクト単位でユーザーやロールを管理
- **クォータ管理**: プロジェクトごとにリソースの上限（インスタンス数、コア数、メモリなど）を設定
- **課金・コスト管理**: プロジェクト単位でリソース使用量を追跡

**プロジェクト、ユーザー、ロールの関係**:

```mermaid
graph TB
    Domain[ドメイン<br/>default]
    Domain --> Project1[プロジェクト1<br/>admin]
    Domain --> Project2[プロジェクト2<br/>service]
    Domain --> Project3[プロジェクト3<br/>engineering]
    
    Project1 --> User1[ユーザー<br/>admin]
    Project2 --> User2[ユーザー<br/>glance]
    Project2 --> User3[ユーザー<br/>nova]
    Project3 --> User4[ユーザー<br/>developer1]
    
    User1 --> Role1[ロール<br/>admin]
    User2 --> Role2[ロール<br/>service]
    User3 --> Role2
    User4 --> Role3[ロール<br/>member]
    
    style Domain fill:#e1f5ff
    style Project1 fill:#fff9c4
    style Project2 fill:#fff9c4
    style Project3 fill:#fff9c4
    style Role1 fill:#ffeb3b
    style Role2 fill:#ffeb3b
    style Role3 fill:#ffeb3b
```

**`admin`プロジェクトと`service`プロジェクト**:

| プロジェクト | 用途                            | 使用するユーザー                                     | 作成方法            |
| ------------ | ------------------------------- | ---------------------------------------------------- | ------------------- |
| `admin`      | 管理者によるOpenStackの管理操作 | `admin`ユーザー（人間の管理者）                      | bootstrapで自動作成 |
| `service`    | OpenStackサービス間の認証・通信 | 各サービスのサービスユーザー（`glance`、`nova`など） | 手動作成が必要      |

> **💡 重要**: `service`プロジェクトは、OpenStackの**慣習的に使用されるプロジェクト名**です。技術的に特別な扱いを受けるわけではありませんが、OpenStackの標準的なインストール手順では、各サービスがこのプロジェクトを使用することが推奨されています。

**`service`プロジェクトの役割**:

1. **OpenStackサービス間認証用**
   - OpenStackの各サービス（Glance、Nova、Neutron、Cinderなど）が、Keystoneに認証する際に使用するプロジェクト
   - 各サービスは`service`プロジェクトに属するサービスユーザー（例: `glance`、`nova`、`neutron`、`cinder`）として認証します
   - **注意**: 技術的には、各サービスは任意のプロジェクトに属するユーザーとして認証できますが、慣習として`service`プロジェクトが使用されます

2. **サービスカタログの管理**
   - 各サービスのエンドポイント（API URL）を`service`プロジェクトのコンテキストで管理します

3. **サービス間通信の認証**
   - サービス間でAPIを呼び出す際の認証に使用されます
   - 例: NovaがGlanceのAPIを呼び出す際、Novaは`service`プロジェクトのコンテキストで認証します

**具体例**:

- **Glance**: Glanceサービスは`service`プロジェクトに属する`glance`ユーザーとしてKeystoneに認証します
- **Nova**: Novaサービスは`service`プロジェクトに属する`nova`ユーザーとしてKeystoneに認証します
- **管理者操作**: 管理者は`admin`プロジェクトに属する`admin`ユーザーとしてOpenStackを管理します

> **📌 参考**: プロジェクトの設計方法については [Part 4 - プロジェクト・マルチテナント設計](04_system_design.md#3-プロジェクトマルチテナント設計) を参照してください。

##### マルチテナントとは何か

**マルチテナント（Multi-Tenant）**は、1つのOpenStack環境で複数の組織や顧客（テナント）が、互いに干渉することなくリソースを使用できる仕組みです。

**マルチテナントの特徴**:

- **リソースの分離**: 各プロジェクトのリソース（VM、ネットワーク、ストレージ）は完全に分離されます
- **セキュリティ**: プロジェクト間でのリソースアクセスは制御されます
- **クォータ管理**: プロジェクトごとにリソース使用量の上限を設定できます
- **課金**: プロジェクト単位でリソース使用量を追跡し、課金できます

**マルチテナントの実現方法**:

- **プロジェクトによる分離**: 各テナント（組織、顧客）を異なるプロジェクトとして作成
- **ネットワーク分離**: NeutronのVXLANやVLANにより、プロジェクト間のネットワークを分離
- **セキュリティグループ**: プロジェクトごとにセキュリティルールを設定

> **📌 参考**: マルチテナント設計の詳細については [Part 4 - プロジェクト・マルチテナント設計](04_system_design.md#3-プロジェクトマルチテナント設計) を参照してください。
